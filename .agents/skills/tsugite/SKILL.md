...
When the selected backend declares `capabilities.native_authoring`, read its backend guide and query the installed runtime schema before composing `native_edit.payload`. If the runtime is missing or incompatible, report the setup requirement; never install it as a side effect of planning or review.

When a user wants to compose in Tsugite and finish editing in a native-authoring backend, keep the Tsugite approval flow and select that backend through `edit.backend`. Read its backend guide, prepare the complete native document/actions and declared outputs before Gate 1, then use the installed runtime schema. The selected backend's guide defines its runtime-specific fields and limits.

- Keep the complete backend payload in the Gate 1 subject. Set `native_edit.mode: replace` only when the payload defines the complete exported edit; otherwise use `extend` so the ordinary timeline remains required.
- Declare local source files under `native_edit.assets` and `native_edit.fonts`; keep them project-relative and make their references visible in the payload. Use `tsugite:request:<request-id>:<video|image|audio>:<1-based-result-index>` when a payload references a future generated result. Tsugite resolves that token after generation and before Gate 2; unresolved bindings fail closed.
- Inspect the Gate 1 payload digest, declared resource list, generated bindings, and code-input hashes/previews. If the payload, declared resource, or source bytes change after approval, re-run the review and obtain a fresh Gate 1 decision.
- For a full native document, review the declared primary output and every sidecar's duration, dimensions, frame rate, codec, alpha, and audio expectations at Gate 1. Inspect the resulting preview, filmstrip, canonical video, and any sidecars at Gate 3.
- After Gate 2 approves the assembled inputs and the Coordinator explicitly starts render, inspect the artifacts named in the selected backend guide together at Gate 3. Diagnostic images do not approve Gate 3.
- Preserve backend-specific limits documented in its guide. Do not install a backend runtime as a side effect of planning or review.
