...
6. Run `bin/pipeline validate --config <project.yaml> --json`. Resolve any `LOCK-E001` before Gate 1. Treat `identity.subject_unlocked` as a plan warning: either confirm identity with the user and set `locked: true`, or leave false and report the risk in plain language.
...
7. **`locked` flag:** Keep `locked: false` (or omit) until the user has confirmed the fixed identity lines (or accepted a deliberate risk). After confirmation, set `locked: true`. Never auto-run credit-burning stress tests to flip the flag.
