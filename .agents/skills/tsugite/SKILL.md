...
14. Treat `worktree_warning.active` from `worktrees --json` as a report-only signal that at least three clean, merged, unlocked, unprotected worktrees are cleanup-ready. Report it, but never count active/blocked worktrees toward the threshold or treat the warning as completion approval. A scheduled host may return `DONT_NOTIFY` while the signal is inactive; it still must not remove anything when active.
