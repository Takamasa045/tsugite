...
- **HyperFrames Studio / WebMCPの操作指定**：[Studio接続手順](docs/hyperframes-studio-webmcp.md) を読む。0.8.75固定の実験的入口。authoring copy を `npm run hyperframes:studio -- <composition-dir>` で開き、ページ内ツールの発見・handle指定の書き込み契約・実ファイルと画像の照合を守る。Studio編集はmanifestへ自動反映されず、pipeline renderでHTMLが再生成されるため、制作run正本を直接編集しない。
- **Editframe backend / preview**：[接続手順](docs/editframe.md)。任意のローカル backend。`npm run editframe:install` で pin 済み runtime を導入し、`project.yaml` の `edit.backend: editframe` から `pipeline render` する。preview は authoring copy のみ。WebMCP / disk-save API は未検証。Gate の代替ではない。
