...
- ユーザーが成果物を明示的に「完成」としたら、Coordinatorは当該タスクの差分だけを確認・テストし、統合可能なコミットとworktree削除計画を報告する。commit・main統合・worktree削除はユーザーの明示承認後に実行する。承認済みでmainがクリーンならmainへ統合して当該worktreeを削除する。mainが他タスクの未コミット変更を含む場合は、それらをstash・rebase・破棄せず、統合可能なコミットを残して具体的な阻害要因を報告する。
