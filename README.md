...
Experimental [HyperFrames Studio WebMCP](docs/hyperframes-studio-webmcp.md) uses the pinned local HyperFrames CLI: `npm run hyperframes:studio -- <composition-dir>`. Tool discovery and some text/style editing runs have succeeded, but inspect can fail even when the preview contains the target, including after reload. Reliable editing is not yet verified; see the linked report before use. Work on an authoring copy: Studio edits do not update the pipeline manifest and a subsequent pipeline render regenerates the HTML. Existing Gates remain required.
