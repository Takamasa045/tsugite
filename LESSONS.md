...
2026-09-10 / HyperFrames Studioの12ツール発見だけでは編集可否を証明できず、0.8.33のset_textは実ブラウザでsignal引数の例外になった / 登録契約とconsumer APIの実行契約、現行ガイドと固定版の選択契約が異なる / 実ブラウザの発見→選択→最小編集→source保存→reload→画像で対応版を決める。0.8.24を互換固定し、初回ID割当後は再読込、書込前は選択を別callで確認する / Chrome 152 native WebMCP・fresh fixtureで実証、docs/reports/hyperframes-studio-webmcp-2026-09-10.md
