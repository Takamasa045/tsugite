...
2026-09-25 / HyperFrames 0.8.75 で旧Studio smokeの選択確認が失敗した / `studio_select`成功後も`isCurrentSelection`がfalseで、書き込みschemaは明示`handle`必須へ変化 / 更新時は実ページが返すschemaを読み、fixtureでhandle指定の文字・色変更、保存、reload、PNG内容まで照合する / Chrome 153の隔離fixtureで確認
...
2026-08-11 / 長尺アーカイブダイジェストのシーン見出しがダサい・小さく・位置が悪いと差し戻された / チャンキーな HIGHLIGHT バッジや右上固定透かしを先に決め、中央→上部の動きとサイズをユーザー確認前に固定しなかった / アーカイブ切り出しダイジェストでは (1) 下帯焼き込み字幕と被らない位置 (2) ガラス薄板+細いアクセントのエディトリアル見出し (3) 各カット冒頭は中央ポップ→上部定着 (4) シリーズ透かし位置は左上を既定候補として見せ、サイズは一回り大きめから提示する / observed

2026-08-12 / still-heroで生成静止画を前面にしたらキャラが消えた・文字が重なった / SceneStillをAbsoluteFill+zIndex全面にし、TitleTagとTopicCardとCaptionを同時表示した / still-heroでは (1) 静止画は枠内divのみで全面AbsoluteFillを使わない (2) キャラは高いzIndexで下部に大きく出す (3) 大きなTitle/Topicカードを重ねず薄いHeader＋静止画下端見出し＋下帯テロップに分ける / observed
...
2026-09-10 / HyperFrames Studioの12ツール発見だけでは編集可否を証明できず、0.8.33のset_textは実ブラウザでsignal引数の例外になった / 登録契約とconsumer APIの実行契約、現行ガイドと固定版の選択契約が異なる / 実ブラウザの発見→選択→最小編集→source保存→reload→画像で対応版を決める。0.8.24を互換固定し、Shadow DOM内の実プレビュー準備後にlook/inspectを別callで実行し、書込前は選択を確認する / Chrome 152 native WebMCP・fresh fixtureで実証、docs/reports/hyperframes-studio-webmcp-2026-09-10.md
2026-09-10 / Studioの保存とreloadが成功してもstudio_frameが編集前PNGを返した / PNG形式だけのsmokeでは古いrender cacheを検出できなかった / 保存済みsourceと画像内容は別に確認し、公式settleMsで画像取得のみを有期限再試行する。固定fixtureでは変更色の画素を検査し、画像更新後にreloadを確認する / 旧画像0画素・新画像1566画素で差を実証、検証scriptを強化
2026-09-10 / WebMCPの実機失敗が文書末尾にだけ記されREADMEと冒頭の成功表現が残った / 個別run成功と全体の受入状態を混同した / 最新の受入状態を入口と記録冒頭で統一し、失敗phase・実DOM・process-tree停止証拠も残す / GitHub反映前の再精査で修正、reload後inspectにも失敗例あり
