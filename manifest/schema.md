...
Optional `fast_edit` is a strict backend-neutral edit-intent object: `version: 1`, five `global` style choices, timestamped `words` with unique IDs, and contiguous `beats` containing seven editing decisions and their `word_ids`. Emphasis IDs must refer to a word in the same beat; all words are assigned once. The final beat must equal the assembled source duration. Executable/component fields and unknown choices are rejected. The compiler preserves source trims and audio timing and emits captions plus the same intent in the EDL. See [Fast Edit mode](../docs/fast-edit.md) for the schema choices and evidence protocol.
