...
describe("agent service remote MCP client", () => {
  it("passes client metadata name=tsugite-agent-service and version=0.11.0 to clientFactory", async () => {
  ...
  it("connects, lists tools, marks undeclared tools blocked, and always closes", async () => {
    ...
    expect(result.blocked_by_policy).toEqual([]);
  ...
  it("puts only undeclared names in blocked_undeclared and declared non-callable in blocked_by_policy", async () => {
    const service: AgentServiceDefinition = {
      ...readOnlyService(),
      tools: [
    ...
    const client = mockClient({
      async listTools() {
        client.state.calls.push("listTools");
        return {
          tools: [
            { name: "search" },
            { name: "submit_inquiry", description: "side effect" },
            { name: "hidden_admin" }
          ]
        };
      }
    ...
    expect(result.blocked_undeclared).toEqual(["hidden_admin"]);
    expect(result.blocked_by_policy).toEqual([
      { name: "submit_inquiry", reason: "side_effect" }
    ]);
    expect(result.observed_tools).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "search", declared: true, callable: true }),
        ...
        expect.objectContaining({ name: "hidden_admin", declared: false, callable: false })
      ])
    );
  });

  it("raceWithAbort settles once, cleans abort listeners, and does not orphan delayed rejects", async () => {
    ...
    try {
      ...
      const delayed = new Promise<string>((_resolve, reject) => {
        rejectDelayed = reject;
      });

      await expect(raceWithAbort(delayed, controller.signal)).rejects.toMatchObject({
        issues: [expect.objectContaining({ code: AGENT_SERVICE_ISSUE_CODES.timeout })]
      });

      // Late rejection must be absorbed by raceWithAbort handlers (no unhandledRejection).
      rejectDelayed(new Error("late orphan reject"));
      await new Promise((resolve) => setImmediate(resolve));
      await new Promise((resolve) => setImmediate(resolve));
      expect(unhandled).toEqual([]);

      // Successful work path: no leftover abort side-effects after settle.
      const live = new AbortController();
      await expect(raceWithAbort(Promise.resolve("done"), live.signal)).resolves.toBe("done");
      live.abort(); // post-settle abort must not throw or spawn unhandled rejections
      await new Promise((resolve) => setImmediate(resolve));
      expect(unhandled).toEqual([]);

      // Abort-while-pending path settles once with timeout.
      const live2 = new AbortController();
      let addCount = 0;
      let removeCount = 0;
      const signal = live2.signal;
