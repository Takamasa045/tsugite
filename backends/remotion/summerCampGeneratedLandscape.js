...
export function SummerCampGeneratedLandscape({ manifest }) {
  ...
  return h(AbsoluteFill, { style: { overflow: "hidden", color: C.paper, fontFamily: SANS, pointerEvents: "none" } },
    ...
    h("div", { style: { position: "absolute", left, top, width: 1920, height: 1080, scale, transformOrigin: "top left" } },
      h(Brand),
      h(JoineryLock, { localFrame }),
      h(Interactive.Div, { name: "Title", style: { position: "absolute", left: 82, right: 82, top: 148, bottom: 110, display: "flex", alignItems: "center", opacity: motion.visible, translate: `${shift}px 0px` } },
...
function Brand() {
  return h(Interactive.Div, { name: "Header", style: { position: "absolute", zIndex: 2, left: 82, top: 54, display: "flex", alignItems: "center", gap: 14 } },
...
function JoineryLock({ localFrame }) {
  ...
  return h(React.Fragment, null,
    ...
    h("div", { style: { position: "absolute", left: 492, bottom: 34, width: 11, height: 27, background: C.coral, opacity: lock, rotate: "45deg" } })
  );
}
...
function Story({ headline, visual, localFrame }) {
  ...
  return h(React.Fragment, null,
    ...
    visual.detail ? h("div", { style: { marginTop: 24, maxWidth: 990, fontSize: 31, fontWeight: 700, lineHeight: 1.42, opacity: detailEnter, translate: `0px ${(1 - detailEnter) * 12}px`, textShadow: "0 2px 15px rgba(0,0,0,.7)" } }, visual.detail) : null,
    (visual.points ?? []).length ? h("div", { style: { display: "flex", flexWrap: "wrap", gap: 12, marginTop: 30, opacity: pointsEnter, translate: `0px ${(1 - pointsEnter) * 10}px` } },
...
function Price({ visual, localFrame }) {
  ...
  return h("div", { style: { display: "grid", gridTemplateColumns: "minmax(0, 1fr) 300px 300px", gap: 18, alignItems: "end", marginTop: 24, maxWidth: 1540 } },
    ...
    h("div", { style: { opacity: cardsEnter, translate: `0px ${(1 - cardsEnter) * 18}px` } },
