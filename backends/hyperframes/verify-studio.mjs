...
assert.equal(version, "0.8.75", "Revalidate the Studio contract before changing the pinned version");
...
async function editCycle(page, handle, color, label) {
  ...
  await call(page, "studio_set_text", { handle, text: color.text });
  const textReadback = await call(page, "studio_inspect", { handle });
  assert.equal(textReadback.handle, handle);
  assert.equal(textReadback.text, color.text);
  assert.equal(textReadback.can.editStyles, true);
  const style = await call(page, "studio_set_style", { handle, styles: { color: color.hex } });
