...
test("ships site-specific metadata, assets, and accessibility styles", async () => {
  ...
  assert.match(page, /alt="Tsugiteランチャーの実画面/);
  assert.match(page, /srcSet="\/launcher-screen\.avif"/);
  ...
  assert.match(css, /\.hero::before\s*\{[^}]*image-set\([^}]*og-background\.avif/s);
  ...
  assert.match(css, /\.hero::before\s*\{[^}]*filter:\s*sepia\(\.18\) saturate\(\.72\)/s);
