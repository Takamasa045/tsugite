...
AIエージェントの制作工程を、2Dノードと木組みの3D工程図で確認するローカルアプリです。和紙、藍、青磁、杉を基調にした「墨庭の設計卓」として、装飾より工程の判読性を優先しています。

2Dランチャーは、Tsugite CLIを定められた操作だけに限定して呼び出すローカルブリッジです。一方、3D Viewerは`project.yaml`・実行状態・Gate/QC成果物をViewer JSONへ変換した、自己完結の静的・閲覧専用成果物のままです。
...
ブラウザの制作棚から案件を選ぶと、`validate` / `plan` / `review` / Gate 1 / `run` / Gate 2 / `render` / Gate 3の工程を2Dノードで確認できます。ランチャーはローカルの保存状態を定期的に再読み込みするため、別ターミナルのCLIが更新した`state`も自動反映されます。ただし、プロバイダーからのイベント配信やフレーム単位のリアルタイム進捗ではありません。

ランチャーから実行できるのは、許可された次の操作だけです。
...
自由なシェルコマンドは入力・実行できません。現在の`state`で許可される操作だけが表示されます。`run`、`render`、Gateの変更はランチャー上の明示確認が必要で、確認後もCLI側のGate前提とCoordinator actor制約を回避できません。実生成の`run`はプロバイダークレジットを消費し、プロンプトや素材を外部送信する場合があります。表示される操作内容を確認してから実行してください。
...
- 2Dランチャーの状態反映は定期再読み込みです。プロバイダーのリアルタイム進捗配信ではありません。
