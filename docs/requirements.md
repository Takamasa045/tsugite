...
- composition付きprojectでは `compose` が最大3件の比較可能な構成案を読み取り専用で生成し、`edit.composition.proposal_id` で選択した1案と現在の解析・brief・reviewをGate 1へ固定する。承認後の `run` だけがsource/output対応を持つ `composition-edl.json` と並べ替え済みmanifestを生成する
...
projectを扱うサブコマンドは `--config <project.yaml>` を受け取り、全サブコマンドが `--json` 出力に対応する。`guides` と `presets` はproject非依存の読み取り専用コマンドなのでconfig不要。
...
- 生成要求は「capability」「model」「connection profile（契約・認証・課金経路）」を分離する。同じmodelに複数のconnectionを登録でき、モデルカタログの存在をconnectionや利用権限の根拠にしない。
- 動画、画像、TTS、音楽、SFX、文字起こしは同じconnection選択契約を使い、capabilityごとの対応有無を宣言する。
- 外部生成でconnection未指定の場合は、project確定と外部実行の前に、対応候補と状態を提示して利用者へ確認する。modelだけが指定され、対応するready connectionが複数ある場合も同様とする。
- サービスと対応modelが両方明示された場合は接続について再質問しない。選択後に別connection、別model、別課金アカウントへ自動fallbackしない。
- connection状態は `ready | needs-verification | needs-setup | not-integrated` で正規化する。transport導入済みだけ、または登録枠があるだけで `ready` にしない。
- ready connectionがない場合は、手持ち素材、手動取り込み、ローカル生成、個別connection設定を案内し、すべてのサービスへの契約を要求しない。
- 新しいMCP、CLI、公式API、ローカルruntimeはconnectionレジストリに追加し、認証、capability/model対応、setup check、見積もり、job追跡、ローカル成果物化、エラー正規化、外部送信範囲を宣言する。宣言が不十分な接続はmanual importとして扱う。
- 外部生成adapterはconnectionを必須とし、未登録adapterをadapter名だけで実行しない。`local-only`免除は外部送信と外部課金を行わないテスト／ローカルadapterに限定する。
- subscription/API keyのintegrated connectionはsetup checkを最低1件持ち、認証方式とsecretを含まない接続契約digestをGate 1承認へ固定する。
- secretはチャット、`project.yaml`、repo内へ記録せず、プロバイダのログイン、OS credential store、または宣言済み環境変数に委譲する。
- 利用者向けTopView接続はMCPとして表示する。`adapters/topview/` は公式HTTPS MCPを呼ぶrepo-local実行互換層とし、Gate 1承認後にだけ実行する。transport到達だけを認証、利用権限、残高確認済みの証明にしない。詳細は `docs/connections.md` を正本とする。
...
- **capabilities.yaml** を必ず持つ:
  ...
  - `validate` は「manifest が要求する機能 ⊆ 選択バックエンドの capabilities」を
    実行前にチェックし、不一致なら拒否する
  - `presets --backend <name> --json` は同じcapabilities契約から登録済みpresentation presetを読み取り専用で列挙する
...
- `project.yaml` の `audio.adapter` で動画生成adapterと独立した音声実行経路を選択する
- BGM/SFXのprompt・開始/終了・音量はGate 1 review対象とし、実行はGate 1後・Gate 2前に限定する
- adapter出力はrun directory内のregular fileに限り、manifest、Gate 2 QC、input digest、run logで固定する
- `hyperframes-media` は公式`media-use`を`--only bgm,sfx`で呼び、ElevenLabsに自動fallbackしない
- HyperFramesで未解決のBGM/SFXがある場合はGate 2前にfail closedとする
...
- Gate 2 の条件付き自動通過（opt-in）: project.yaml に `gates.gate_2.auto_pass: qc_ok_no_new_assets` を明示したローカル素材プロジェクトに限り、実測クレジット 0、新規生成アセット 0 件、QC issue 0 件のすべてを満たしたときだけ、人間承認と同一の承認 digest を算出して `decision_source: auto_qc` で承認を記録する。1 つでも外れた場合は現行どおり `awaiting_gate_2` で停止し、`run` の出力に `gate_2_auto_pass_blocked_reason` を返す。generation.requests を持つプロジェクトは validate 時点で opt-in を却下する。自動通過後も状態が `rendering` になるだけで render 自体は開始せず、Gate 1 と Gate 3 の人間承認は不変とする
...
4. **昇格承認**: 反映先、変更内容、検証方法が揃った時点で昇格案を作り、ランチャーに
   ...
   ランチャーは起動時に承認待ちを読み、専用workflow `tsugite-learning-promotion-review`
   由来の `pending` 件数だけを未読風バッジとピックアップで表示する。手動案と別workflowは
   通常の棚に残す。承認または見送りの追記で待ちを解消する。
   ブラウザ通知権限は要求せず、デスクトップ通知を送信しない
...
- ランチャーはデスクトップ通知、常駐サービス、外部通知先を持たず、prompt、template、check、
  運用ruleを書き換える権限も持たない
- ランチャーから再生成するViewerは、実行中だけ有効な権限 `0700` のprivate一時領域へ書き、
  projectの出力pathへ書き戻さない。終了時に同じ実体と確認できた一時領域だけを削除する
- Codex / Claude自動化は他の自動化の通知器ではなく、「好み・学び」の昇格候補を承認待ちにする専用自動化とする。
  ランチャーの起動・停止と独立して実行し、実行元を記録する。ブラウザ、独自デスクトップ通知、常駐サービス、外部通知先を使わず、完了通知はhost標準機能に限定する
- 自動化は各1回で、反映先・変更内容・検証方法・根拠が揃う重複のない候補を最大3件まで選び、
  既存の `pipeline feedback` CLI経由で対象projectの `feedback.jsonl` へ `pending` を追記する。承認前後とも共有sourceを自動変更しない
- 同じ `key` の未解消 `pending` 、または `key` / 反映先 / 変更内容 / 検証方法 / 根拠が一致する既存案は、
  実行元と判断状態にかかわらず再追記しない。詳細契約は `docs/automations/learning-promotion-review.md` を正本とする
