...
- Codex自動化は全自動化の通知器ではなく、「好み・学び」の昇格候補を承認待ちにする専用自動化とする。
  ランチャーの起動・停止と独立して実行し、ブラウザ、デスクトップ通知、常駐サービス、外部通知先を使わない
...
- 同じ `key` の未解消 `pending` 、または `key` / 反映先 / 変更内容 / 検証方法 / 根拠が一致する既存案は、
  判断状態にかかわらず再追記しない。詳細契約は `docs/automations/learning-promotion-review.md` を正本とする
