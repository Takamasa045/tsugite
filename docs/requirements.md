...
2. **状態**: feedbackは `observed`（初回記録）→ `recurring`（同じ `key` の反復）→
   `promoted`（人間が再利用変更を承認）→ `verified`（後続出力で改善確認）として追跡する。
   反復検出は昇格候補を示すだけで、自動昇格しない
...
4. **実行チェックへ昇格**: 人間の承認後、コードで判定できるルールは
...
5. **QA へ昇格**: Gate 2 / Gate 3 で判定すべきルールは check、report schema、
   fixtures、tests を同時に更新する
6. **契約文書へ昇格**: コード化できない判断ルール（演出・構成系）は
   `.agents/skills/tsugite/SKILL.md` / CLAUDE.md の運用ルール節へ転記する
7. **検証**: 昇格後の出力で同じ問題が再発しないことを確認し、根拠を持つ記録だけを
   `verified` とする
