...
- `analyze` はCoordinatorの明示実行で、元素材を変更せず `dist/<run-id>/analysis/` にAPI-freeなraw analysis、editorial proposal、agent handoffを生成する。request単位で複数のoffline adapterを選択でき、local-media-analysisはFFmpeg無音検出、local-whisper-analysisは明示済みローカル`.pt`による文字起こし・フィラー候補・章・抽出的要約・英訳字幕を担当する。削除やmanifest反映は行わない。
...
- `bin/pipeline` CLI（doctor / validate / plan / analyze / review / run / render。Shitate連携時のみ任意の `shitate-import`）
