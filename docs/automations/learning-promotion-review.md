...
CodexまたはClaude Codeは、ローカル初回セットアップと `doctor` が成功した後、次の実質的な提案に入る前に一度だけ次を確認する。

> 初回設定が完了しました。任意で、ローカルの「好み・学び」を定期レビューし、Codex または Claude の標準通知で承認待ちを知らせる自動化も設定しますか？（設定する／今回はしない）

「設定する」と明示された場合だけ、主系を Codex / Claude Desktop・Cowork / Claude Code から一つ選び、実行頻度を確認して登録する。「今回はしない」の場合は、その初回セットアップ中に再確認しない。Browser・OSの通知権限、独自Desktop通知、Slack、メールなどの外部通知先は設定せず、選んだhostの標準通知だけを使う。
