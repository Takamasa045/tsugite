...
| 項目 | 契約 |
|---|---|
| 状態 | 価格正本未設定の間は **preflight-only / blocked**。実送信準備完了とは表示しない |
| 初期 scope | MiniMax-H3 **last-frame-only** のみ。first-frame 付与・同画像複製・T2V 降格・他 mode 推測は禁止 |
...
| テスト | repo-local mock transport を DI。外部 DNS/HTTP/provider を呼ばない |
