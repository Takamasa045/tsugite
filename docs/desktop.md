...
> 状態: 一般配布終了。リポジトリのソフトウェア version は **0.17.0**。この Electron パッケージの `package.json` version は lockfile 都合で 0.6.0 のまま。macOS / Windows向けインストーラーの新規公開・更新・利用者サポートは行いません。通常利用はGitHubのリポジトリをCodex / Claude Codeで開き、ブラウザ版のローカルランチャー／Viewerを使ってください。
