...
`knowledge/video-models/` は、動画モデル別のプロンプト知識を機械可読 YAML で保持します。収録: PixVerse、Kling、Seedance、**MiniMax H3（公式 source catalog）**。

カタログは計画用データです。カタログが存在しても、同名 adapter、API 接続、契約、利用権限、残クレジットが存在するとは限りません。`guides` は読み取り専用で、外部APIを呼びません。

`minimax-h3` カタログは公式 MiniMax source のみを pin します（Skill commit、API V2、prompt guide、CLI、MCP 非対応の明記）。PixVerse の limits / sources と混ぜません。Skill 本文は vendoring せず `redistribution: link-only` / `license_status: unverified` / `review_after` を保持します。公式に根拠がない Tsugite 判断は source-backed official rule として表示しません。
...
対応する `input_mode` は `text-to-video`、`image-to-video`、`transition`、`reference`、および H3 向けの `last-frame-to-video` / `first-last-frame-to-video` です。カタログに mode recipe が無い入力モードは `input-mode-unsupported` になります。

これらはGate 1前の助言です。coreはpromptを書き換えず、`run` / `render` の承認条件も変更しません。実行能力は評価しません（`execution_capability: not-evaluated`）。

`minimax-h3` で section 順・shot 時刻・speaker ID・locked dialogue・L2VA/FL2VA alignment を決定的に組み立てる場合は、カタログ助言とは別に [H3 Prompt Director](./h3-prompt-director.md)（Creative IR + compiler workflow v2）を使う。H3 compiler / IR / Gate artifact は **Tsugite 独自契約**であり、通常 prompt の自動書換え機構ではない。
