...
3. **DNS publicness**: production MCP session は **pre-connect 1 回** DNS を解決し、次を拒否する: loopback / private / link-local / CGNAT / ULA / multicast / unspecified / documentation（`2001:db8::/32`, `3fff::/20`） / site-local / discard（`100::/64`） / benchmarking（`2001:2::/48`） / ORCHID v1+v2（`2001:10::/28`, `2001:20::/28`） / NAT64 well-known（`64:ff9b::/96` の埋込 private） / NAT64 local-use（`64:ff9b:1::/48` は prefix 全体 reject） / 6to4 埋込 private / IPv4-mapped・deprecated IPv4-compatible（`::/96` non-mapped）の埋込 private・loopback。Allowlist は **exact endpoint**（scheme/origin/port/path）のみ。hostname-only 判定は提供しない。`createAllowlistedFetch` 単体の default は **per-request 再検査**だが、`defaultTransportFactory` は precheck 後 `skipDns=true` で二重解決を避ける。**DNS TOCTOU**（解決後に応答が変わる）は限界として残る。主要 trust boundary は固定 Cloudflare / Workers host。
